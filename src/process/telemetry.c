    ...
    uint8_t * pucTelemetryData, uint32_t ulTelemetryDataLength )
{
    ...
    // non-nested attributes
    {
        ...
        const char* timestamp_utc = "tbd";//GetUtcTimeInString();
    ...
    // nested errors
    {
        ...
        if( skid_data.skid_state == Safe_State )
        {
            xResult = AzureIoTJSONWriter_AppendBeginObject( &xWriter );
            configASSERT( xResult == eAzureIoTSuccess, xResult );
            ...
            xResult = AzureIoTJSONWriter_AppendPropertyWithStringValue( &xWriter, ( uint8_t * ) sampleazureiotTELEMETRY_PREVIOUS_STATE, lengthof( sampleazureiotTELEMETRY_PREVIOUS_STATE ),
                                                                        ( uint8_t * )sequence_state_stringified[skid_data.previous_skid_state], strlen(sequence_state_stringified[skid_data.previous_skid_state]));
            configASSERT( xResult == eAzureIoTSuccess, xResult );
        ...
        if( unit_data.unit_state == Safe_State )
        {
            xResult = AzureIoTJSONWriter_AppendBeginObject( &xWriter );
            configASSERT( xResult == eAzureIoTSuccess, xResult );
            ...
            // @todo need to implement current and previous states
            xResult = AzureIoTJSONWriter_AppendPropertyWithStringValue( &xWriter, ( uint8_t * ) sampleazureiotTELEMETRY_PREVIOUS_STATE, lengthof( sampleazureiotTELEMETRY_PREVIOUS_STATE ),
                                                                        ( uint8_t * )sequence_state_stringified[unit_data.previous_unit_state], strlen(sequence_state_stringified[unit_data.previous_unit_state]));
            configASSERT( xResult == eAzureIoTSuccess, xResult );
