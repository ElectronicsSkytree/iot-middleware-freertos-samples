...
// For testing
// #define TESTING_IN_SINGLE_TASK 1
 ...
 */
static BaseType_t vIoTTelemetryTask( void );
 ...
 *      - Direct Methods
 */
// static void vIoTCloudMessageTask( void );
                                        ...
                                        void * pvContext )
{
    ...
    switch( pxMessage->xMessageType )
    {
        case eAzureIoTHubPropertiesRequestedMessage:
            ...
            /**
             * @todo After the separate cloud msg task is re-enabled, uncomment this
            */
...
static void prvIoTTelemetryTask( void * pvParameters )
{
    ...
    */
    while(1) {
        ...
        configPRINTF( ("prvIoTTelemetryTask wait for relevant bits\r\n") );
    ...
    while(1)
    {
        ...
        */
        ...
        // SKID_iot_status_t skid_data = {0};
        // UNIT_iot_status_t unit_data = {0};
...
static void prvIoTBackendConnectionTask( void * pvParameters )
{
    ...
    while( 1 )
    {
         ...
         *       in cloud msg task.
         * @todo Cloud message is handled in telemetry task currently. Later to use
         *       separate cloud message task after fixing it.
         */
        prvSubscribeCloudStuff();

#ifdef TESTING_IN_SINGLE_TASK
        int p = 0;
        prvIoTCloudMessageTask(&p);
        ...
        // Start receiving cloud messages
        if( pdFALSE == vIoTTelemetryTask() )
        {
            LogError( ( "vIoTTelemetryTask failed") );
        }
 ...
 *       controller_communication_task.
 */
BaseType_t vIoTTelemetryTask( void )
{
#ifndef TESTING_IN_SINGLE_TASK
